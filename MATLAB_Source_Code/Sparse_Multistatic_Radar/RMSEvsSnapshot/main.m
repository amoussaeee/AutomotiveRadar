...
%% SNR
SNR=150; %Transmitted
eb_vector_loc        = [0.000015 0.0000220 0.0000290 0.000035];
eb_vector_Doppler_1  = [0.000003 0.0000037 0.0000043 0.0000050];
...
Loop_Num=1;
Methods_num=2;
MSE_loc=zeros(Methods_num,Q_length);
MSE_velocity=zeros(Methods_num,Q_length);
MSE_Doppler_1=zeros(2,Q_length);
MSE_Doppler_2=zeros(2,Q_length);



% Compressive Sensing buffers
loc_Bias_Store_CS         = zeros(K*Loop_Num,Q_length);
velocity_Bias_Store_CS    = zeros(K*Loop_Num,Q_length);
Doppler_Bias_Store_CS_1   = zeros(K*Loop_Num,Q_length);
Doppler_Bias_Store_CS_2   = zeros(K*Loop_Num,Q_length);
...
% MUSIC buffers
loc_Bias_Store_SS         = zeros(K*Loop_Num,Q_length);
velocity_Bias_Store_SS    = zeros(K*Loop_Num,Q_length);
Doppler_Bias_Store_SS_1   = zeros(K*Loop_Num,Q_length);
Doppler_Bias_Store_SS_2   = zeros(K*Loop_Num,Q_length);
...
%eb_test=norm(Z_1);
% noise_test_loc_1=norm(W_1(:,Q(1)),'fro');
% W_1_Doppler_1=W_1.';
% noise_test_Doppler_1=norm(W_1_Doppler_1(:,Q(1)),'fro');
% 
% noise_test_loc_4=norm(W_1(:,Q(2)),'fro');
% W_1_Doppler_4=W_1.';
% noise_test_Doppler_4=norm(W_1_Doppler_4(:,Q(2)),'fro');
...
%noise_test_pair=norm(W_1(:));

%% Proposed %%
% Solve the location  problem using l21 minimisation with M_p=8;
[est_location_signal,solver_status_loc]=l21_minimisation_multistatic([Z_1(:,1:Q),Z_2(:,1:Q)],P_g1,P_g2,eb_loc,G_p,Q);
...
loc_Bias_Store_CS(loop_index,Q_index)=loc_Bias;
...
% Solve the Doppler problem using l21 minimisation with N_p=2 and L_p=4
% from sensor 1
[est_Doppler_signal_1,solver_status_Doppler]=l21_minimisation(Z_1_t(:,1:Q*2),V_g,eb_Doppler_1,G_d,2*Q);
...
Doppler_Bias_Store_CS_1(loop_index,Q_index)=Doppler_Bias_1;
% 
...
% Solve the Doppler problem using l21 minimisation with N_p=2 and L_p=4
% from sensor 2
[est_Doppler_signal_2,solver_status_Doppler]=l21_minimisation(Z_2_t(:,1:Q*2),V_g,eb_Doppler_2,G_d,2*Q);
if strcmp(solver_status_Doppler,'Solved')==0
...
Doppler_Bias_Store_CS_2(loop_index,Q_index)=Doppler_Bias_2;
...
velocity_Bias_Store_CS(loop_index,Q_index)=velocity_Bias;
...
est_Doppler_signal_1=MUSIC(Z_1_t_comp(:,1:Q*2),V_g,Num_Targets,G_d);
...
Doppler_Bias_Store_SS_1(loop_index,Q_index)=Doppler_Bias_1;


% Sensor 2
est_location_signal_2=MUSIC(Z_2_comp(:,1:Q),P_g2,Num_Targets,G_p);
...
Doppler_Bias_Store_SS_2(loop_index,Q_index)=Doppler_Bias_2;

% Location Average
[est_loc_Value, loc_Bias,True_Index_Record_loc] = Location_Estimation_Bias_Average(Num_Targets, buffer_loc_1,buffer_loc_2, Target_Positions_pol, Target_Positions_car, pol_grid, x_vec, y_vec,I,J); 
loc_Bias_Store_SS(loop_index,Q_index)=loc_Bias;
[est_R_k, est_theta_k]=car2pol(est_loc_Value(:,1),est_loc_Value(:,2));
...
est_parameters_final=(est_parameters_1+est_parameters_2)/2;
velocity_Bias = (est_parameters_final(:,3)-Target_rav(:,3)).^2;
velocity_Bias_Store_SS(loop_index,Q_index)=velocity_Bias;
...
%% Calculating RMSE %%
  ...
  for q=1:Q_length
MSE_loc(1,q)      = sum(loc_Bias_Store_CS(:,q))/(Num_Targets*Loop_Num);
MSE_loc(2,q)      = sum(loc_Bias_Store_SS(:,q))/(Num_Targets*Loop_Num);
MSE_velocity(1,q) = sum(velocity_Bias_Store_CS(:,q))/(Num_Targets*Loop_Num);
MSE_velocity(2,q) = sum(velocity_Bias_Store_SS(:,q))/(Num_Targets*Loop_Num);


MSE_Doppler_1(1,q) = sum(Doppler_Bias_Store_CS_1(:,q))/(Num_Targets*Loop_Num); 
MSE_Doppler_2(1,q) = sum(Doppler_Bias_Store_CS_2(:,q))/(Num_Targets*Loop_Num); 
MSE_Doppler_1(2,q) = sum(Doppler_Bias_Store_SS_1(:,q))/(Num_Targets*Loop_Num); 
MSE_Doppler_2(2,q) = sum(Doppler_Bias_Store_SS_2(:,q))/(Num_Targets*Loop_Num); 
...
%save RMSEvsSnapshot_test_1000.mat
%toc
